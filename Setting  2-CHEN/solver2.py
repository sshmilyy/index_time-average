...
class TimeVaryingWhittleSolver:
    ...
    def _solve_rvi(self, nu, h_init=None):
        ...
        for _ in range(50):
            h_rolled = np.roll(h, shift=-1, axis=0)

            # 💡 黑魔法 3：爱因斯坦张量求和 (Einstein Summation)
            # 一行代码，直接消灭原本的 `for t in range(24)` 循环！
            # 'tsaj,tj->tsa'：让 4 维概率矩阵 和 2 维价值矩阵 在底层的 C 语言里瞬间完成全部相乘与求和
            future_v = np.einsum('tsaj,tj->tsa', self.P_mat, h_rolled, optimize=True)

            q_vals = R_nu + future_v
            h_new = np.max(q_vals, axis=2)
