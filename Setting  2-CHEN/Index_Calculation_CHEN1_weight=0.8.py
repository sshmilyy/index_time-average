...
@njit(fastmath=True)
def run_pvi_numba(nu, T, P_mat, R_mat, actions_arr, V_init):
    """
    Numba 加速版的周期性值迭代
    """
...
@njit(fastmath=True)
def get_q_diff_numba(s_idx, t, mode_k, nu, V_converged, T, P_mat, R_mat, actions_arr):
    """
...
@njit(parallel=True)
def compute_index_table_parallel(T, NUM_STATES, MAX_CHARGE, P_mat, R_mat, actions_arr):
    """
    并行计算所有状态的索引
    """
    # 结果容器
    ...
    for s_idx in prange(NUM_STATES):
        ...
        for t in range(T):
            ...
            for k in range(int(MAX_CHARGE)):
                ...
                # 预检 nu=0
                V_0 = run_pvi_numba(0.0, T, P_mat, R_mat, actions_arr, V_cache)
                diff_0 = get_q_diff_numba(s_idx, t, k, 0.0, V_0, T, P_mat, R_mat, actions_arr)
                ...
                for _ in range(20):
                    ...
                    V_mid = run_pvi_numba(mid, T, P_mat, R_mat, actions_arr, V_cache)
                    # 更新 cache 以加速下一次 PVI
                    V_cache = V_mid

                    diff = get_q_diff_numba(s_idx, t, k, mid, V_mid, T, P_mat, R_mat, actions_arr)
...
if __name__ == '__main__':
    ...
    # 这里的第一次调用会触发 JIT 编译
    t0 = time.time()
    TABLE = compute_index_table_parallel(period, NUM_STATES, MAX_CHARGE, P_mat, R_mat, actions_arr)
    t1 = time.time()
