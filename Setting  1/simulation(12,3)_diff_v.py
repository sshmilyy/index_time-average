...
def get_time_varying_prob(t, period=24):#v2
    t_in_period = t % period

    if t_in_period <= 6:  # For hours 0-6, probability is 0
        return 0.0
    elif t_in_period <= 12:  # For hours 7-12, probability increases linearly from 0.2 to 0.7
        return 0.2 + (0.5 * (t_in_period - 7) / 5.0)
    elif t_in_period <= 20:  # For hours 12-20, probability is 0.7
        return 0.7
    else:  # For hours 21-23, probability decreases linearly from 0.7 to 0
        return 0.7 - (0.7 * (t_in_period - 21) / 3.0)
def get_time_varying_prob(t, period=24):#v3
    t_in_period = t % period

    if t_in_period <= 5:  # For hours 0-5, probability is 0
        return 0.0
    elif t_in_period <= 12:  # For hours 6-12, probability increases linearly from 0.2 to 0.7
        return 0.9
    elif t_in_period <= 19:  # For hours 13-19, probability is 0.7
        return 0.5
    else:  # For hours 20-23, probability decreases linearly from 0.7 to 0
        return 0
'''
...
def transition_probability_simu(current_state, action, current_t, arrival_seq):
    """
    Deterministic transition with Dynamic Assignment.
    1. Decay current cars.
    2. Identify empty spots.
    3. Fill spots with Poisson arrivals.
    4. Excess arrivals leave (Reneging).
    """
    ...
    for i in range(N):
        ...
        if new_state[r_idx] < action[i]:
            ...
            raise ValueError(f"Action {action[i]} exceeds remaining charge {new_state[r_idx]} at index {i}")
...
def process_sequence(original_seq):
    """
    Converts variable-length Poisson arrivals into a fixed N x T grid
    for the Clairvoyant solver.
    Logic:
    - Map arrivals to slots 0..N-1.
    - If a slot is occupied by a car arriving at t with duration l,
      block that slot for [t+1, ... t+l-1].
    - Fill gaps with 0.
    """
    T_len = len(original_seq)

    # Dictionary to store the 'virtual' deterministic schedule
    dict_r = {}
    dict_l = {}

    # Track when each slot n becomes free (time index)
    slot_free_time = {n: 0 for n in range(N)}

    # Temporary dense access to arrivals (handling list of lists)
    # We only care about the first N arrivals max, as system capacity is N

    for t in range(T_len):
        arrivals_at_t = original_seq[t]
        arrival_idx = 0  # Pointer to the next available car in the arrival list

        for n in range(N):
            # 1. Check if this slot is blocked by a previous car
            if t < slot_free_time[n]:
                # Slot busy -> No new arrival can enter here in the CV model
                dict_r[(t, n)] = 0
                dict_l[(t, n)] = 0
            else:
                # Slot is free -> Try to assign next car from the arrival list
                if arrival_idx < len(arrivals_at_t):
                    ...
                    # No more cars arriving at this instant
                    dict_r[(t, n)] = 0
                    dict_l[(t, n)] = 0
...
def cvt_cts_policy(arrival_seq, alpha, penalty_weight, p_0, total_power):
    # 1. Process sequence (和你现在的逻辑保持一致)
    dict1, dict2 = process_sequence(arrival_seq)

    r = {(n + 1, t + 1): v for (t, n), v in dict1.items()}
    l = {(n + 1, t + 1): v for (t, n), v in dict2.items()}

    # 2. Active Mask (确保只在车停着的时候充电)
    active_mask = {(n, t): 0 for n in range(1, N + 1) for t in range(1, T + 1)}
    for n in range(1, N + 1):
        for t in range(1, T + 1):
            if r[n, t] > 0 and l[n, t] > 0:
                start_t = t
                # 注意：这里和 process_sequence 保持对齐
                end_t = min(t + l[n, t], T + 1)
    ...
    # 3. 目标函数 (Objective)
    # 关键：显式使用传入的 penalty_weight
    # 注意：penalty 项只在有车到达的那一刻 (r > 0) 计算一次，避免重复扣分
    obj_expr = gp.LinExpr()

    for t in range(1, T + 1):
        for n in range(1, N + 1):
            # 收益部分：每充 1 单位电的净收益
            obj_expr += (alpha - p_0) * w[n, t]

            # 惩罚部分：只在车到达的时间点计算 (当 r[n,t] > 0 时)
            # 如果在每个时刻都减去 penalty，会导致这一辆车的 penalty 被重复计算多次！
            if r[n, t] > 0:
                charged_sum = gp.quicksum(w[n, s] for s in range(t, min(t + l[n, t], T + 1)))
                obj_expr -= penalty_weight * (r[n, t] - charged_sum) * (r[n, t] - charged_sum)
    ...
    for n in range(1, N + 1):
        for t in range(1, T + 1):
            if active_mask[n, t] == 0:
                model.addConstr(w[n, t] == 0, name=f"no_car_{n}_{t}")
    ...
    if model.status == GRB.OPTIMAL:
        return model.getAttr('X', w), model.objVal
    else:
        return {}, 0
...
def idx_xu(r: int, l: int) -> float:
    ...
    # R <= (L-1) * MAX_CHARGE
        ...
        return alpha - p_0 + delta_f(r - (l - 1) * MAX_CHARGE)
...
def index(r: int, l: int, t: int, i: int) -> float:
    ...
    if r > l * MAX_CHARGE:
        if 0 <= i <= MAX_CHARGE - 1:
            return alpha - p_0 + delta_f(r - (l - 1) * MAX_CHARGE - i)
        else:
            return 0
...
if __name__ == "__main__":
    ...
    N = 10
    for penalty_weight in [0.2,0.4]:
        for power_ratio in [0.3,0.45,0.6]:
            ...
            for exp_id in range(NUM):
                ...
                _, cvt_reward = cvt_cts_policy(current_arrival_seq,alpha, penalty_weight, p_0, total_power)
    ...
    df.to_excel("poisson_results(12,3).xlsx", index=False)
