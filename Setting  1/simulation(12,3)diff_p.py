...
'''
def get_time_varying_p0(t, period=24):
    t_in_period = t % period
    if t_in_period <= 6:
        return 1.0
    elif t_in_period <= 12:
        return 1.0 - (0.5 * (t_in_period - 6) / 6.0)
    elif t_in_period <= 20:
        return 0.5 + (1.0 * (t_in_period - 12) / 8.0)
    else:
        return 1.5 - (0.5 * (t_in_period - 20) / 4.0)
...
def get_time_varying_p0(t, period=24):
    ...
    if t_in_period <= 6:
        ...
        return 0.8 + (0.4 * (t_in_period - 12) / 8.0)
    else:
...
def get_time_varying_p0(t, period=24):
    ...
    if t_in_period <= 6:
        ...
        return 1.0 - (0.1 * (t_in_period - 6) / 6.0)
    elif t_in_period <= 20:
        return 0.9 + (0.2 * (t_in_period - 12) / 8.0)
    else:
        return 1.2 - (0.1 * (t_in_period - 20) / 4.0)
...
def reward_function(current_state, action,t):
    ...
    rewards = [
        (min(r, a) * (alpha - p_t) - (f(max(0, r - a)) if l == 1 else 0))
...
def cvt_cts_policy(arrival_seq):
    ...
    # 目标函数
    model.setObjective(
        gp.quicksum(gp.quicksum((alpha - get_time_varying_p0(t, period=24)) *
                                w[n, t] - gamma[n, t] * (r[n, t] - gp.quicksum(
            w[n, s] for s in range(t, min(t + l[n, t], T + 1)))) ** 2
    ...
    for t in range(1, T + 1):  # 注意：Gurobi里的t是从1开始的
        # 计算当前时刻对应的折扣因子 gamma^(t-1)
        # 因为仿真里 t=0 是第一刻，这里 t=1 是第一刻
        discount = beta ** (t - 1)

        for n in range(1, N + 1):
            # 收益部分：带折扣
            obj_expr += discount * (alpha - p_0) * w[n, t]

            # 惩罚部分：带折扣
            # Penalty 发生在车到达的时刻 (t)，所以也乘同样的 discount
            if r[n, t] > 0:
                charged_sum = gp.quicksum(w[n, s] for s in range(t, min(t + l[n, t], T + 1)))
                obj_expr -= discount * penalty_weight * (r[n, t] - charged_sum) * (r[n, t] - charged_sum)
...
def idx_xu(r: int, l: int, t) -> float:
    ...
    # R <= (L-1) * MAX_CHARGE
        ...
        return alpha -  get_time_varying_p0(t, period=24)+ delta_f(r - (l - 1) * MAX_CHARGE)
...
if __name__ == "__main__":
    ...
    for penalty_weight in [0.2,0.4,0.6,0.8]:
        for VEHICLE_GEN_PROB in [0.5, 0.7,0.9, 1]:
            for power_ratio in [0.3,0.45,0.6,0.75,0.9,1.05,1.2]:
    ...
    df.to_excel("simulation_results(12,3)diff_p(0.9,1,1.1).xlsx", index=False)
