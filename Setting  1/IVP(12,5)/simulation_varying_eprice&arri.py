...
if __name__ == "__main__":
    ...
    for penalty_weight in [0.2,0.4,0.6,0.8]:
        file_name =f"whittle_index_multimode_time_varying ePrice_penal={penalty_weight}.npy"
        print(f"Loading index file: {file_name}")
        INDEX_TABLE = np.load(file_name)

        for power_ratio in [0.3,0.5,0.7,0.9,11]:
    ...
    df.to_excel("poisson_results(12,5)_penal.xlsx", index=False)
